...
회의·취침 중 등 잠시 알림을 끄고 싶을 때 PC 재설치 없이 카카오 채널에서 직접 토글할 수 있습니다.

### 1회 페어링 (관리자에게 받은 코드 사용)

관리자가 API 키와 함께 **6자리 페어링 코드**를 줍니다 (10분 유효). 카카오 채널 챗봇에서:

```
/연동 ABC123
```

→ "✅ 연동 완료" 메시지가 오면 끝. 이후 토글 가능.
...
|--------|------|
| `/알림 켜기` | 알림 ON (기본값) |
| `/알림 끄기` | 알림 OFF — Claude Code 알림이 도착하지 않음 |
| `/알림 상태` | 현재 ON/OFF 상태 확인 |

OFF 상태에서도 PC 클라이언트는 정상 동작 (NAS 까지 요청은 가도 카톡 발송이 스킵됨). 다시 켜면 즉시 알림 재개.

> 페어링을 1회도 안 한 상태면 **기본 ON** 으로 동작하므로 즉시 알림이 옵니다. 토글이 필요한 사람만 페어링하면 됩니다.

### 코드가 만료/분실됐다면

10분 안에 입력 못 했으면 만료. 관리자에게 새 코드 요청:
```
관리자 측: python3 add_tenant.py --data-dir /volume1/docker/scripts pair <your-tenant-id>
```
